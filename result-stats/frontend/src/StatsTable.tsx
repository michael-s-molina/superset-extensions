...
import { common, theme } from '@apache-superset/core';

const { useTheme } = theme;
type SupersetTheme = theme.SupersetTheme;
import { ColumnStats, ResultStats } from './types';

const TYPE_CONFIG: Record<common.GenericDataType, { label: string; color: string }> = {
  [common.GenericDataType.Numeric]: { label: 'number', color: 'green' },
  [common.GenericDataType.String]: { label: 'string', color: 'blue' },
  [common.GenericDataType.Temporal]: { label: 'date', color: 'purple' },
  [common.GenericDataType.Boolean]: { label: 'boolean', color: 'orange' },
};
...
const TABLE_COMPONENTS: Record<common.GenericDataType, React.FC<{ data: ColumnStats[] }>> = {
  [common.GenericDataType.Numeric]: NumericTable,
  [common.GenericDataType.String]: StringTable,
  [common.GenericDataType.Temporal]: TemporalTable,
  [common.GenericDataType.Boolean]: BooleanTable,
};

interface TypeSectionProps {
  type: common.GenericDataType;
  data: ColumnStats[];
}
...
const TYPE_ORDER = [
  common.GenericDataType.Numeric,
  common.GenericDataType.String,
  common.GenericDataType.Temporal,
  common.GenericDataType.Boolean,
] as const;

const StatsTable: React.FC<StatsTableProps> = ({ stats }) => {
  const theme = useTheme();

  const dataByType = stats.columns.reduce<Partial<Record<common.GenericDataType, ColumnStats[]>>>(
    (acc, col) => {
